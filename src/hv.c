...
avl_alloc_tree(avl_compare_t cmp, avl_freeitem_t freeitem) {
	return avl_init_tree((avl_tree_t *) malloc(sizeof(avl_tree_t)), cmp, freeitem);
}
...
setup_cdllist(double *data, int d, int n)
{
    ...
    head->next = (struct dlnode **) malloc( d * (n+1) * sizeof(dlnode_t*));
    head->prev = (struct dlnode **) malloc( d * (n+1) * sizeof(dlnode_t*));
    head->tnode = (avl_node_t *) malloc ((n+1) * sizeof(avl_node_t));
...
static void delete_node (dlnode_t *nodep, int dim, double * bound __variant3_only)
{
    int i;
...
double fpli_hv(double *data, int d, int n, const double *ref)
{
    ...
    if (n != nn) {
    	printf("Points went from %d to %d, bad reference!\n", nn, n);
    }
    if (n == 0) { 
        hyperv = 0.0;
    } else if (n == 1) {
